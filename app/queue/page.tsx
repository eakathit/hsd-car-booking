// app/queue/page.tsx — ตารางคิวรถ real-time + ยกเลิกได้เฉพาะของตัวเอง
"use client";
...
import { useLiff } from "../../lib/liff-context"; // ✅ เพิ่ม
...
export default function QueuePage() {
  const { user } = useLiff(); // ✅ ดึง user เพื่อเช็คสิทธิ์

  const [bookings, setBookings]       = useState<Booking[]>([]);
  const [loading, setLoading]         = useState(true);
  const [filter, setFilter]           = useState<Filter>("date");
  const [fDate, setFDate]             = useState(todayStr());
  const [cancelling, setCancelling]   = useState<string | null>(null); // bookingId ที่กำลัง cancel
  ...
  const q = filter === "date"
    ? query(
        ...
        where("useDate", "==", fDate),
        where("status", "in", ["booked", "active", "completed"]), // ✅ ตัด cancelled ออก
  ...
  // ── ตรวจสิทธิ์ยกเลิก — เฉพาะ booked + เจ้าของ ──────────
              ...
              {bookings.map((b) => {
                ...
                return (
                  <div
                    key={b.id}
                    className="bk-card"
                    style={{ opacity: b.status === "cancelled" ? 0.6 : 1 }}
                  >
                    {/* Left color strip */}
                    <div
                      className="strip"
                    ...
                    <div className="card-body">
                      ...
                      <div className="row-between" style={{ marginBottom: 10 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          ...
                          {/* 🏷️ badge ของตัวเอง */}
                          {isMine && (
                            <span className="mine-badge">ของฉัน</span>
                          )}
                        </div>
                        <span
                      ...
                      {/* ── Row 6: Cancel button — เฉพาะ booked + เจ้าของ ── */}
                      {canCancel(b) && (
                        <button
                        ...
                        >
                          {isCancelling
                            ? <><span className="spin-sm" /> กำลังยกเลิก…</>
                            : "✕ ยกเลิกการจอง"}
                        </button>
                      )}
                    </div>
