...
async function getCarAvailability(
  useDate: string, startTime: string, endTime: string, carIds: string[],
  ...
  for (const id of carIds) {
    result[id] = { available: true, bookerName: "", startTime: "", endTime: "" };
  }
  for (const d of snap.docs) {
    const b = d.data() as Booking;
    if (startTime < b.endTime && endTime > b.startTime) {
      result[b.carId] = { available: false, bookerName: b.bookerName, startTime: b.startTime, endTime: b.endTime };
    }
  }
  return result;
}
...
export default function BookPage() {
  ...
  const { cars, loading: carsLoading } = useCars(true); // ✅ ดึงเฉพาะรถที่ active

  const [step, setStep]                   = useState(0);
  const [isLoadingCars, setIsLoadingCars] = useState(false);
  const [isSubmitting, setIsSubmitting]   = useState(false);
  const [submitted, setSubmitted]         = useState(false);
  const [availability, setAvailability]   = useState<Record<string, { available: boolean; bookerName: string; startTime: string; endTime: string }>>({});
  ...
  const goStep1 = async () => {
    ...
    try {
      const av = await getCarAvailability(form.useDate, form.startTime, form.endTime, cars.map(c => c.id));
      setAvailability(av);
  ...
  if (submitted && selectedCar) return (
    <><Styles />
      ...
      <div style={{ background: "#f0f4f8", minHeight: "100vh", paddingBottom: 40 }}>
        ...
        <div style={{ maxWidth:440,margin:"0 auto",padding:"14px 14px 0" }}>
            ...
            {STEPS.map((label,i) => (
              <div key={i} style={{ display:"flex",alignItems:"center",flex:i<STEPS.length-1?1:"none" }}>
                <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:3 }}>
                  <div style={{ width:26,height:26,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,background:i<step?"#16a34a":i===step?"linear-gradient(135deg,#1d4ed8,#1e3a5f)":"#e2e8f0",color:i<=step?"white":"#94a3b8",boxShadow:i===step?"0 3px 10px rgba(29,78,216,.35)":"none" }}>
                    {i < step ? "✓" : i+1}
                  </div>
          ...
          <div className="card">
              ...
              <div className="step-in">
                ...
                {ok0 && (
                  <div style={{ display:"flex",alignItems:"center",gap:8,background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:12,padding:"10px 13px",marginBottom:14 }}>
                    ⏱ <span style={{ color:"#15803d",fontWeight:600,fontSize:13 }}>ระยะเวลา: <strong>{getDuration(form.startTime,form.endTime)}</strong></span>
                  </div>
                )}
                {form.startTime && form.endTime && form.startTime>=form.endTime && (
                  <div className="err">⚠️ เวลาเข้าต้องมากกว่าเวลาออก</div>
                )}
                <button className="btn-primary" disabled={!ok0||isLoadingCars||carsLoading} onClick={goStep1}>
                  {isLoadingCars ? <><span className="spin-sm" /> กำลังตรวจสอบรถว่าง…</> : "ดูรถว่าง →"}
                </button>
              </div>
            )}
            ...
            {step===1 && (
              <div className="step-in">
                <div style={{ display:"flex",gap:8,background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:12,padding:"10px 13px",marginBottom:16 }}>
                  <div style={{ flex:1 }}><p style={{ fontSize:10,color:"#64748b",margin:0 }}>วันที่ใช้รถ</p><p style={{ fontSize:13,fontWeight:700,color:"#1e3a5f",margin:0 }}>{fmtDate(form.useDate)}</p></div>
                  <div style={{ flex:1 }}><p style={{ fontSize:10,color:"#64748b",margin:0 }}>เวลา</p><p style={{ fontSize:13,fontWeight:700,color:"#1e3a5f",margin:0 }}>{form.startTime} – {form.endTime}</p></div>
                  <button onClick={() => setStep(0)} style={{ background:"none",border:"none",cursor:"pointer",color:"#1d4ed8",fontSize:11,fontWeight:700,alignSelf:"center",padding:0 }}>แก้ไข</button>
                </div>

                <div className="fg">
                  <label className="fl">เลือกรถยนต์ *</label>
                  {carsLoading ? (
                    <div style={{ textAlign:"center",padding:"20px 0",color:"#94a3b8",fontSize:13 }}>กำลังโหลดรถ…</div>
                  ) : cars.length===0 ? (
                    <div style={{ textAlign:"center",padding:"20px 0",color:"#94a3b8",fontSize:13 }}>ยังไม่มีรถในระบบ</div>
                  ) : (
                        ...
                        return (
                          <div key={car.id} onClick={() => isFree && set("carId",car.id)} style={{ display:"flex",flexDirection:"column",borderRadius:16,overflow:"hidden",border:`2px solid ${isSel?SEL_BORDER:isFree?"#e2e8f0":"#fecaca"}`,cursor:isFree?"pointer":"not-allowed",background:isSel?SEL_BG:isFree?"white":"#fff8f8",boxShadow:isSel?SEL_SHADOW:"0 2px 8px rgba(0,0,0,.07)",transition:"all .2s",position:"relative" }}>
                            <div style={{ width:"100%",aspectRatio:"4/3",background:isSel?"#dbeafe":isFree?"#f1f5f9":"#fee2e2",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",position:"relative" }}>
                              ...
                              {!isFree && <div style={{ position:"absolute",inset:0,background:"rgba(0,0,0,0.38)",display:"flex",alignItems:"center",justifyContent:"center" }}><span style={{ fontSize:15,fontWeight:700,color:"white",background:"#ef4444",borderRadius:20,padding:"4px 14px",boxShadow:"0 2px 8px rgba(0,0,0,.3)" }}>ไม่ว่าง</span></div>}
                              {isSel && <div style={{ position:"absolute",top:8,right:8,width:26,height:26,borderRadius:"50%",background:SEL_BORDER,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 2px 8px rgba(0,0,0,.25)" }}><span style={{ color:"white",fontSize:13,fontWeight:700 }}>✓</span></div>}
                            </div>
                            <div style={{ padding:"10px 12px 12px",textAlign:"center" }}>
                              ...
                              {isFree
                                ...
                                : <div><span style={{ display:"inline-block",fontSize:12,fontWeight:700,color:"#ef4444",background:"#fee2e2",border:"1px solid #fecaca",borderRadius:20,padding:"3px 12px",marginBottom:4 }}>✕ ไม่ว่าง</span><p style={{ fontSize:11,color:"#94a3b8",margin:0 }}>{av.startTime}–{av.endTime}</p></div>}
                            </div>
                          </div>
                        );
                      })}
                ...
                <div style={{ display:"flex",gap:8 }}>
                  ...
                  <button className="btn-primary" style={{ flex:1 }} disabled={!ok1} onClick={() => setStep(2)}>ถัดไป: รายละเอียด →</button>
                </div>
              </div>
            )}
            ...
            {step===2 && selectedCar && (
              <div className="step-in">
                <div style={{ display:"flex",alignItems:"center",gap:10,background:selectedCar.colorLight||"#f1f5f9",border:`1px solid ${selectedCar.color}44`,borderRadius:12,padding:"10px 13px",marginBottom:16 }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <p style={{ fontSize:13,fontWeight:700,color:"#1e3a5f",margin:0 }}>{selectedCar.name} <span style={{ fontWeight:400,color:"#64748b",fontSize:12 }}>({selectedCar.plate})</span></p>
                  </div>
                ...
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>
                  <div className="fg"><label className="fl">📍 จาก *</label><input type="text" value={form.fromLocation} onChange={(e) => set("fromLocation",e.target.value)} placeholder="Haru" className="inp" /></div>
                  <div className="fg"><label className="fl">🏁 ถึง *</label><input type="text" value={form.toLocation} onChange={(e) => set("toLocation",e.target.value)} placeholder="DCI, Daikin…" className="inp" /></div>
                </div>
                <div className="fg"><label className="fl">📋 สำหรับ *</label><input type="text" value={form.purpose} onChange={(e) => set("purpose",e.target.value)} placeholder="Install, Test run, Meeting…" className="inp" /></div>
                <div className="fg"><label className="fl">👤 ผู้ใช้รถ *</label><input type="text" value={form.driverName} onChange={(e) => set("driverName",e.target.value)} placeholder="ชื่อผู้ขับ" className="inp" /></div>
