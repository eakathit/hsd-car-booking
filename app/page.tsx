...
function pct(mins: number) { return `${Math.max(0,Math.min(100,((mins-DAY_START)/(DAY_END-DAY_START))*100)).toFixed(1)}%`; }
...
export default function Dashboard() {
  ...
  const { cars, loading: carsLoading } = useCars(true); // ✅ ดึงจาก Firestore
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .booking-bar{position:absolute;height:100%;border-radius:6px;display:flex;align-items:center;padding:0 8px;overflow:hidden;cursor:pointer;border:none;transition:filter .15s,transform .15s}
        .booking-bar:hover{filter:brightness(1.1);transform:scaleY(1.05)}
        .booking-bar-label{font-size:11px;font-weight:600;color:white;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;pointer-events:none}
        .no-booking{font-size:12px;color:#94a3b8;text-align:center;line-height:36px}
      ...
      <div className="page">
        <header className="sticky-header">
          ...
          <Link href="/queue" style={{ marginLeft:'auto',fontSize:12,color:'#1d4ed8',fontWeight:600,textDecoration:'none',background:'#eff6ff',padding:'6px 12px',borderRadius:20,border:'1px solid #bfdbfe',whiteSpace:'nowrap' }}>
            ดูคิวทั้งหมด →
          </Link>
        </header>
        ...
        <div className="section">
          <div className="card">
            <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8 }}>
              ...
              <input type="date" value={selectedDate} className="date-input" onChange={(e) => setSelectedDate(e.target.value)} />
            </div>
            <p style={{ fontSize:12,color:'#64748b',marginBottom:16 }}>{formatThDate(selectedDate)}</p>
                ...
                return (
                  <div key={car.id} className="car-row">
                    <div className="car-label">
                      ...
                      {carBookings.length===0 && <span style={{ marginLeft:'auto',fontSize:11,color:'#16a34a',fontWeight:600 }}>✓ ว่าง</span>}
                    </div>
                    <div className="timeline-wrap">
                      {carBookings.length===0 && <div className="no-booking">— ว่างทั้งวัน —</div>}
                      {carBookings.map((b) => {
                        const s = toMinutes(b.startTime);
                        const e = toMinutes(b.endTime);
                        return (
                          <button key={b.id} className="booking-bar"
          ...
          {!loading && bookings.length>0 && (
            <div className="card">
              <p className="card-title" style={{ marginBottom:12 }}>📋 รายการจองวันนี้ ({bookings.length} รายการ)</p>
              {bookings.map((b) => {
                  ...
                  <div key={b.id} className="booking-list-item">
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start' }}>
                        ...
                        <span style={{ fontSize:11,background:'#f1f5f9',color:'#475569',borderRadius:20,padding:'2px 8px',fontWeight:600 }}>{b.startTime} – {b.endTime}</span>
                      </div>
                      <p style={{ fontSize:12,color:'#64748b',margin:'2px 0 0' }}>{car?.name} · {b.fromLocation} → {b.toLocation}</p>
          ...
          {!loading && !carsLoading && bookings.length===0 && (
            <div className="card" style={{ textAlign:'center',padding:'32px 20px' }}>
              <p style={{ fontSize:32,marginBottom:8 }}>🚗</p>
              <p style={{ fontSize:14,fontWeight:600,color:'#1e3a5f',margin:'0 0 4px',fontFamily:'Prompt,sans-serif' }}>ยังไม่มีการจองในวันนี้</p>
